...
func newConsumer(amqpUri, exchange, exchangeType, queueName, key, ctag string, messages chan []byte) (*Consumer, error) {
	var err error
	c := &Consumer{
		conn:    nil,
		channel: nil,
	...
	go handle(deliveries, c.done, ctag, messages)

	return c, err
}

func handle(deliveries <-chan amqp.Delivery, done chan error, tag string, messages chan []byte) {
